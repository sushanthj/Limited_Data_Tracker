...
def train(args):
    ...
    model = Network(num_classes=len(train_dataset.classes), drop_rate=args.stochastic_depth_prob, expand_factor=4, config=config)
...
def test(args):
    ...
    # Initialize the model
    model = Network(num_classes=2, drop_rate=0.5, expand_factor=4, config=config)
    model = model.to(DEVICE)
    optimizer = torch.optim.AdamW(model.parameters(), lr=config['lr'], betas=(0.9, 0.999), weight_decay=0.05)
    gamma = 0.6
    milestones = [10,20,40,60,80]
